...
def train(args):
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        if rank == 0:
            ...
            with (Path(args.log_path) / "MMFA150_3_shot.txt").open("a") as f:
                if best_epoch:
                    f.write('Epoch %d: Train loss %.3f, Train_Aux_loss %.3f, Train MAE: %.3f, Val loss %.3f, Val_Aux_loss %.3f, Val MAE: %.3f, lr: %.6f best \n'%(epoch, train_loss.item(), aux_train_loss.item(), train_ae.item() / len(train), val_loss.item(), aux_val_loss.item(), val_ae.item() / len(val), scheduler.get_last_lr()[0]))
                else:
                    f.write('Epoch %d: Train loss %.3f, Train_Aux_loss %.3f, Train MAE: %.3f, Val loss %.3f, Val_Aux_loss %.3f, Val MAE: %.3f, lr: %.6f \n'% (epoch, train_loss.item(), aux_train_loss.item(), train_ae.item() / len(train), val_loss.item(), aux_val_loss.item(), val_ae.item() / len(val), scheduler.get_last_lr()[0]))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('MMAF', parents=[get_argparser()])
